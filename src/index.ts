...
/**
 * フェーズ1: GAS実行環境でGemini APIの動作を検証するデバッグ関数。
 */
...
/**
 * フェーズ2: GAS実行環境でJina AI Readerのフェッチ動作を検証するデバッグ関数。
 */
// biome-ignore lint/correctness/noUnusedVariables: GAS entry point
...
/**
 * フェーズ3: GeminiのダミーデータでNotionへの書き込みを検証するデバッグ関数。
 */
// biome-ignore lint/correctness/noUnusedVariables: GAS entry point
...
/**
 * フェーズ4: Jina→Gemini→Notionの全体フローを検証するデバッグ関数。
 */
// biome-ignore lint/correctness/noUnusedVariables: GAS entry point
function testRun() {
  const { geminiModel, geminiApiKey, notionDbId, notionAccessToken } = getConfig();
