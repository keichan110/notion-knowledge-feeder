...
> **The Sage Sherpa for Your Work and Life**

日々の仕事と生活に関わることを賢くアシストする Google Apps Script システム。

名前の由来は **Sherpa（伴走者）× IA（Intelligent Assistant）**。賢者の知性と伴走者の献身で、あなたの日常に寄り添う。

## 機能

| 機能 | 概要 |
|---|---|
| 記事要約（article-ingest） | iOS ショートカットやトレンドフィードから受けた URL を Gemini API で要約し、Notion のナレッジ DB に自動保存する |
| Newsletter ダイジェスト（gmail-digest） | 前日に届いた Newsletter メールをまとめ、Slack へダイジェスト投稿する |
| ラベル整理（gmail-label-cleanup） | アーカイブ済みメールに残った運用ラベルを自動で外す |
...
GAS のスクリプトプロパティ（API キー・Slack トークンなど）と時間ベーストリガーの登録手順は [docs/setup/google-app-script-setup.md](docs/setup/google-app-script-setup.md) を参照してください。
