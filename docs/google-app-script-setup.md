...
| `NOTION_API_KEY` | ✅ | Notion Integration の API キー | [Notion の「インテグレーション」設定](https://www.notion.so/my-integrations) で作成し、`secret_...` で始まるトークンをコピー |
...
Notion データベースへの書き込みに使う Integration トークンです。

1. [https://www.notion.so/my-integrations](https://www.notion.so/my-integrations) を開く
2. **「New integration」** をクリック
3. 名前（例: `notion-knowledge-feeder`）を入力して作成
4. 表示された **「Internal Integration Token」**（`secret_...`）をコピー
5. **Notion データベースのページを開き、右上「…」→「コネクト先」からこのインテグレーションを追加** することを忘れずに行う
