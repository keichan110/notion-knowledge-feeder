...
- **gmail-digest** — 前日ウィンドウ内に届いた `Newsletter` ラベルのメールをまとめて1通のSlackダイジェストにする。**ダイジェスト型**（設計確定・未実装。ADR-0002）。
...
ダイジェスト型がGmailを集計する**対象期間**。トリガーは当日7時台に動き、対象は**前日の暦日（JST 0:00〜23:59）**。Gmailの日付検索（`after:`/`before:`）の粒度に合わせて**暦日単位**とし、時刻での絞り込みはしない。対象が0件でも「0件」をSinkへ通知する（その場合Geminiは呼ばない）。
